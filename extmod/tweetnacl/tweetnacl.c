...
static mp_obj_t tweetnacl_crypto_sign(mp_obj_t sm) {
  return mp_obj_new_bool(1);
}
static MP_DEFINE_CONST_FUN_OBJ_1(tweetnacl_crypto_sign_obj, tweetnacl_crypto_sign);


static const mp_rom_map_elem_t tweetnacl_module_globals_table[] = { 
  { MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR__tweetnacl) },
...
const mp_obj_module_t mp_module_tweetnacl = {
  .base = { &mp_type_module },
  .globals = (mp_obj_dict_t *)&tweetnacl_module_globals,
};

MP_REGISTER_MODULE(MP_QSTR__tweetnacl, mp_module_tweetnacl)
